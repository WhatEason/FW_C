...
{
	I2C_Start ();
	I2C_Send_Byte((MPU_ADDR<<1)|0) ;//发送写命令
	//等待应答
	if (I2C_Wait_Ack ())
	{
		I2C_Stop ();
...
u8 MPU_Read_Byte(u8 reg)
{
	...
	I2C_Wait_Ack ();
	I2C_Start ();
	I2C_Send_Byte ((MPU_ADDR<<1)|1);//发送读命令
	I2C_Wait_Ack ();
	data=I2C_Read_Byte (0);//读数据，主机不应答
	I2C_Stop ();
...
*Vertion       	: v1.0
*Date						:20170211
*Description 	 	: MPU6050（IIC）连续读
*Input  :			addr：器件地址 		reg：要读取的寄存器地址
...
*Vertion       	: v1.0
*Date						:20170211
*Description 	 	: MPU6050(IIC)连续写
*Input  :				addr：器件地址 reg：寄存器地址
								len：写入长度  buf：数据区
*Output :
*Return :				0：正常 其他：错误代码
*************************************************************/
...
*Vertion       	: v1.0
*Date						:20170211
*Description 	 	: 设置MPU6050的采样率(假定Fs=1KHz)
...
/*************************************************************
*Function Name 	: 得到加速度值(原始值)
*Auther        	: 
*Vertion       	: v1.0
*Date						:
*Description 	 	: ax,ay,az:加速度x,y,z轴的原始读数(带符号)
*Input  :			
*Output :
*Return :				0：成功 其他：错误代码
*************************************************************/
